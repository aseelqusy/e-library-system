...
class AdminController extends Controller {
    ...
    public function storeBook(): void {
        ...
        if (!empty($coverUpload['error'])) {
            error_log('Book cover upload failed: ' . $coverUpload['error']);
        ...
        if (!empty($pdfUpload['error'])) {
            if (!empty($coverUpload['path'])) {
                $this->deleteUpload($coverUpload['path']);
            }
            error_log('Book PDF upload failed: ' . $pdfUpload['error']);
        ...
        try {
            ...
            setFlash('error', 'Failed to add book.');
        }
        $this->redirect('admin/books');
    }
    ...
    public function updateBook(): void {
        ...
        try {
            ...
            setFlash('error', 'Failed to update book.');
        }
        $this->redirect('admin/books');
    }
    ...
    public function updateUserRole(): void {
        ...
        try {
            ...
            $this->json(['success' => true, 'message' => 'Role updated.']);
        } catch (Throwable $e) {
    ...
    public function approveReservation(): void {
        ...
        try {
            Borrow::updateStatus($id, 'active', null, date('Y-m-d'), date('Y-m-d', strtotime('+30 days')));
            Book::adjustAvailable((int)$borrow['book_id'], -1);
